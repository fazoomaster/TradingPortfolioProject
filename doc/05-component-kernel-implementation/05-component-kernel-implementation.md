...
The TradingJournal component will be represented using a
`Sequence<Trade>`.

This structure was selected because the journal logically
models an ordered collection of trades where trades are
added and removed from the end. The Sequence component
support  insertion and removal at arbitrary positions, which allows the
most recently added trade to be removed in constant time by accessing the last index.

### Convention

- trades is not null.
- trades contains only valid Trade objects.
- trades stores trades in insertion order.

### Correspondence

- this = trades
- The TradingJournal abstract value is represented by the
  ordered sequence of Trade objects stored in trades.
